...
function MessagesAdmin() {
  ...
  return (
    <div>
      <h1 className="font-display font-black text-3xl">Contact Messages</h1>
      ...
      <div className="mt-6 rounded-2xl border border-border bg-card overflow-x-auto">
        {filtered.length === 0 ? (
          ...
          <table className="w-full text-sm min-w-[640px]"><thead className="bg-muted"><tr>
            <th className="text-left p-3">From</th><th className="text-left p-3">Subject</th>
            ...
            <tr key={r.id} className="border-t border-border">
              <td className="p-3 font-medium">{r.name}<div className="text-xs text-muted-foreground font-normal">{r.email}</div></td>
              ...
              <td className="p-3"><button onClick={() => openView(r)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded hover:bg-muted"><Eye className="size-3" /> View</button></td>
            </tr>
          ))}</tbody></table>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 grid place-items-center p-4" onClick={() => setViewing(null)}>
          <div className="bg-background rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-border sticky top-0 bg-background">
              <div>
                <h2 className="font-display font-bold text-xl">{viewing.subject}</h2>
                <div className="text-xs text-muted-foreground">{viewing.name} · {viewing.email}</div>
              </div>
            ...
            <div className="p-5 space-y-4 text-sm">
              ...
              <a href={`mailto:${viewing.email}?subject=${encodeURIComponent(`Re: ${viewing.subject}`)}`} className="inline-flex items-center gap-2 h-9 px-3 rounded-full border border-border text-sm hover:bg-muted">
