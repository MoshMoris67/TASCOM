...
export function FloatingActions() {
  ...
  return (
    <>
      {/*
        Rendered as a sibling of the button column, not inside it. The column is
        `fixed` with `z-40`, and a positioned element with a z-index of its own
        creates a stacking context — so the panel's `z-50` was being resolved
        *within* z-40 and could never rise above the header at z-50. Moving it
        out is what lets its own z-index mean anything.
      */}
      ...
      <div className="fixed bottom-5 right-5 z-40 flex flex-col gap-3">
        ...
        */}
          <span
            className={cn(
              "absolute -top-1 -right-1 rounded-full bg-flag-black px-1.5 py-0.5",
              "text-[10px] font-bold uppercase leading-none tracking-wide text-flag-yellow",
              // The chip sits half off the button, so it lands on the page
              // background as often as on the button. `--flag-yellow` is a warm
              // cream, near-white — as a fill it vanished against the page. Navy
              // fill with a cream label reads on the button, on white and in dark
              // mode; the halo keeps it separated from all three.
              "shadow-md ring-2 ring-white/80 dark:ring-white/25",
            )}
          >
            AI
          </span>
